...
def ws_with_investor(ws):
    ...
    inv_dir = ws / "investors" / "demo" / "portfolios" / "core"
    ...
    profile = {
        "investor": {
            "name": "示例投资者",
            "risk_level": "C3",
            "capital_cny": 70852.05,
        }
    }
    (ws / "investors" / "demo" / "profile.yaml").write_text(
    ...
    portfolio = {
        ...
        "assets": [
            {"ticker": "000002", "name": "广发远见智选C", "type": "offshore_fund"},
            {"ticker": "000001", "name": "平安银行", "type": "stock"},
            {"ticker": "600000", "name": "沪深300ETF", "type": "etf"},
            {"ticker": "510050", "name": "科创50ETF", "type": "etf"},
        ],
    }
...
class TestSingleInvestor:
    ...
    def test_portfolio_tickers(self, ws_with_investor):
        ...
        assert set(port["tickers"]) == {"000002", "000001", "600000", "510050"}
...
class TestLedgerScanning:
    ...
    def test_unique_tickers(self, ws_with_ledger):
        ledger = discover_workspace(str(ws_with_ledger))["data"]["ledger"]
        assert set(ledger["unique_tickers"]) == {"000002", "000001", "600000", "510050"}
    ...
    def test_corrupt_line_warning(self, ws_with_investor):
        ...
        ledger_dir.mkdir(parents=True)
        with open(ledger_dir / "transactions.jsonl", "w", encoding="utf-8") as f:
            f.write('{"tx_id": "tx-001", "ticker": "000001", "status": "confirmed"}\n')
            f.write("NOT VALID JSON\n")
            f.write('{"tx_id": "tx-002", "ticker": "600000", "status": "confirmed"}\n')
...
class TestFullIntegration:
    ...
    def test_full_discovery(self, ws_full):
        ...
        assert data["is_initialized"] is True
        assert len(data["investors"]) == 1
        assert data["investors"][0]["id"] == "demo"
        assert data["investors"][0]["portfolios"][0]["id"] == "core"
