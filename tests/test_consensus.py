...
class TestRecordDecision:
    ...
    def test_record_creates_file(self, store):
        """记录应创建 JSONL 文件"""
        d = _make_decision("reasonix", "000001", "buy")
        store.record(d)
    ...
    def test_load_agent_decisions(self, store):
        """加载某 Agent 的决策"""
        store.record(_make_decision("reasonix", "000001", "buy"))
    ...
    def test_load_ticker_decisions(self, store):
        """加载某标的的多 Agent 决策"""
        store.record(_make_decision("reasonix", "000001", "buy"))
        store.record(_make_decision("gemini", "000001", "buy"))
        store.record(_make_decision("reasonix", "510310", "hold"))
        decisions = store.load_ticker_decisions("000001")
        assert len(decisions) == 2
...
class TestConsensus:
    ...
    def test_consensus_achieved(self, store, engine):
        ...
        store.record(_make_decision("reasonix", "000001", "buy"))
        store.record(_make_decision("gemini", "000001", "buy"))
        store.record(_make_decision("claude", "000001", "sell"))

        result = engine.check_consensus("000001")
    ...
    def test_consensus_not_achieved(self, store, engine):
        """无多数 → 未达共识"""
        store.record(_make_decision("reasonix", "000001", "buy"))
        store.record(_make_decision("gemini", "000001", "sell"))
        store.record(_make_decision("claude", "000001", "hold"))
    ...
    def test_same_agent_latest_only(self, store, engine):
        """同一 Agent 只取最新决策"""
        store.record(_make_decision("reasonix", "000001", "buy", timestamp="2026-06-08T10:00:00"))
        store.record(_make_decision("reasonix", "000001", "sell", timestamp="2026-06-08T11:00:00"))
        store.record(_make_decision("gemini", "000001", "sell", timestamp="2026-06-08T10:00:00"))
...
class TestRankAgents:
    ...
    def test_ranking(self, store, engine):
        ...
        for _ in range(5):
            store.record(_make_decision("reasonix", "000001", "buy"))
        for _ in range(3):
            store.record(_make_decision("gemini", "000001", "hold"))
