...
**现象**：
```
调用: record_nav_tool(investor="三尺生", portfolio="main", nav=1, 
      total_assets=70852.05, positions_value=7448.36, cash=63403.69,
...
**现象**：
```
调用: reconcile_tool(investor="三尺生", portfolio="main")
报错: "配置文件不存在: investors\三尺生\profile.yaml"
```
...
**真正问题**：测试时不知道正确的 investor ID（用了 `"三尺生"` 而非 `"example"`），导致路径错误。
...
**现象**：
- 调用 `get_portfolio_tool(investor="三尺生", portfolio="main")` → 报错"配置文件不存在"
...
record_nav_tool(investor="example", portfolio="demo",
                nav=1.0, total_assets=70852.05, positions_value=7448.36,
...
**根因**：测试时使用了错误的 investor ID（`"三尺生"` 而非 `"example"`），
...
2.  get_nav_history_tool(investor="三尺生")         → 0 条记录
3.  get_nav_snapshot_tool(investor="三尺生")        → ❌ 配置文件不存在
