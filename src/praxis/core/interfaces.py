...
# ═══════════════════════════════════════════════════════════════
# P0-2 市场数据采集传输协议（v3.6 新增）
# ═══════════════════════════════════════════════════════════════

@runtime_checkable
class MarketDataTransport(Protocol):
    """⚠️ @deprecated: P0 数据解耦后不再被 MCP server 使用，仅保留用于测试 mock

    市场数据采集传输协议 — 由 MCP server 层注入实现。

    声明 5 个维度的异步数据获取方法，供 MarketWeeklyCollector 并行调用。
    每个方法独立容错，返回统一 dict 结构。
    """

    async def fetch_index_trend(self, start_date: str, end_date: str, index_code: str) -> dict:
        """大盘趋势：周涨跌幅、量能变化、均线位置

        Args:
            start_date: 周起始日期 YYYY-MM-DD
            end_date: 周结束日期 YYYY-MM-DD
            index_code: 大盘指数代码（如 "000300"）

        Returns:
            {"weekly_change_pct": float, "volume_trend": str, "ma_positions": dict, "error": str|None}
        """
        ...

    async def fetch_sector_rotation(self, start_date: str, end_date: str) -> dict:
        """题材轮动：涨幅 TOP10、跌幅 TOP5、连续上榜热点
        ...
        Returns:
            {"top_gainers": list, "top_losers": list, "consecutive_hot": list, "error": str|None}
        """
        ...

    async def fetch_fund_flow(self, start_date: str, end_date: str) -> dict:
        """资金流向：主力周净额、北向周净额、ETF 净流入 TOP5
        ...
        Returns:
            {"main_force_net": float, "north_bound_net": float, "etf_inflow_top5": list, "error": str|None}
        """
        ...

    async def fetch_sentiment(self, start_date: str, end_date: str) -> dict:
        """情绪温度：日均涨跌停比、日均成交额、周波动率
        ...
        Returns:
            {"avg_limit_up_down_ratio": float, "avg_turnover": float, "weekly_volatility": float, "error": str|None}
        """
        ...

    async def fetch_macro_events(self, start_date: str, end_date: str) -> dict:
        """宏观事件：本周重要政策/事件摘要
        ...
        Returns:
            {"events": list[dict], "error": str|None}
        """
        ...
